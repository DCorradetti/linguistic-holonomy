...
observed ratio and $\rho^{h+1}$, over the seven values of $\rho$
from $1$ down to $0.5$,
is $0.0023$, $0.0023$, $0.0022$ and $0.0021$ for $h=0,1,2,3$, and
$0.0011$ for the exponential scheme. At $\rho = 0.5$ the observed ratios
...
% ---------------------------------------------------------------------
% PLACEHOLDER.  This subsection is completed from
% run_exp_indicators_v1_* and run_exp_analysis_v1_* once the corpus run
% has terminated.  Nothing is to be written here that is not read off a
% manifest.
% ---------------------------------------------------------------------

\subsection{Reproducibility}\label{sec:repro}

All computations are performed by five scripts under
\texttt{2.~Scripts/Article\_LLW/}, documented in a local
\texttt{README.md}, and every run writes a fresh timestamped directory
under \texttt{7.~Results/Article\_LLW/} containing its outputs, its log
and a manifest recording the command, the environment, the seeds, the
model revisions, the hashes of the outputs, the numerical conclusions
and the known limitations. No GPU is required and none was used. The
watermark keys are ours, so every detection statistic reported here is
